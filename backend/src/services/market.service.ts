...
import { SP500_SYMBOLS_UNIQUE } from './sp500-symbols';
...
export class MarketService {
  ...
  async getMultipleStocks(symbols: string[], retryCount: number = 0): Promise<StockData[]> {
    const MAX_RETRIES = 3;
  ...
  async getMarketOverview(): Promise<MarketOverviewData[]> {
    ...
    cacheService.set(cacheKey, overview, 10 * 60 * 1000);
  ...
  async getHeatmapData(): Promise<StockData[]> {
    ...
    console.log(`📊 Generating heatmap data for ${SP500_SYMBOLS_UNIQUE.length} stocks...`);
    const allResults = SP500_SYMBOLS_UNIQUE.map(symbol => generateMockStockData(symbol));
    
    // Cache for 1 hour
    cacheService.set(cacheKey, allResults, 60 * 60 * 1000);
  ...
  async getMarketQuotes(symbols: string[]): Promise<StockData[]> {
  ...
  async getHistoricalData(symbol: string, period: string = '1mo'): Promise<any> {
    try {
      ...
      // Cache the result
      cacheService.set(cacheKey, historicalData);
