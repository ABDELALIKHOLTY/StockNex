...
class MarketDataCache {
    constructor() {
        this.allStocksCache = new Map();
        this.lastUpdateTime = 0;
        this.updateInProgress = false;
        this.TOP_STOCKS_BY_SECTOR = 20;
    }
    /**
    ...
    async getStockData(symbol) {
        ...
        try {
            const data = await market_service_1.marketService.getStockData(symbol);
            if (data) {
     ...
     */
    async getStocksBySymbols(symbols) {
        return Promise.all(symbols.map(symbol => this.getStockData(symbol))).then(results => results.filter((stock) => stock !== null));
    }
    /**
    ...
    async getMarketOverview() {
        ...
        cache_service_1.cacheService.set(cacheKey, data, 5 * 60 * 1000); // Cache 5 minutes
        return data;
    }
    /**
    ...
    async getHeatmapData() {
        ...
        cache_service_1.cacheService.set(cacheKey, allStocks, 10 * 60 * 1000); // Cache 10 minutes
...
exports.marketDataCache = new MarketDataCache();
