...
const market_data_cache_1 = require("./services/market-data-cache");
...
const generateJwt = (user) => {
    return (0, jsonwebtoken_1.sign)({ id: user.id, email: user.email }, "JWT_SECRET");
};
...
app.get("/user/me", auth_1.authenticate, async (req, res) => {
    try {
        if (!req.user) {
...
app.get("/api/market/overview", async (req, res) => {
    try {
        const data = await market_data_cache_1.marketDataCache.getMarketOverview();
        res.json(data || []);
    }
    catch (error) {
...
app.get("/api/market/heatmap", async (req, res) => {
    try {
        const data = await market_data_cache_1.marketDataCache.getHeatmapData();
        res.json(data || []);
    }
    catch (error) {
...
app.post("/api/market/quotes", async (req, res) => {
    try {
        ...
        const data = await market_data_cache_1.marketDataCache.getStocksBySymbols(symbols);
...
function generateDescription(companyName, sector) {
...
app.get("/api/market/details/:symbol", async (req, res) => {
    try {
        ...
        try {
            ...
            const queryOptions = {
                modules: ['price', 'summaryDetail', 'defaultKeyStatistics', 'calendarEvents', 'financialData', 'assetProfile', 'industryTrend']
            };
            const result = await yahooFinance.quoteSummary(upperSymbol, queryOptions);
            if (!result) {
            ...
            try {
                ...
                const chartResponse = await axios_1.default.get(chartUrl, {
                    timeout: 10000,
...
// Track user login
app.post("/users/:userId/login", auth_1.authenticate, async (req, res) => {
    try {
...
app.get("/admin/users/:userId/stats", auth_1.authenticate, async (req, res) => {
    try {
        const userId = parseInt(req.params.userId);
...
app.get("/admin/users/:userId", auth_1.authenticate, async (req, res) => {
    try {
        const userId = parseInt(req.params.userId);
        const requestUser = req.user;
        if (!requestUser) {
...
app.get("/users/:userId/watchlist", auth_1.authenticate, async (req, res) => {
    try {
        const userId = parseInt(req.params.userId);
        const requestUser = req.user;
...
// Add to watchlist
app.post("/users/:userId/watchlist", auth_1.authenticate, async (req, res) => {
    try {
        const userId = parseInt(req.params.userId);
...
app.delete("/users/:userId/watchlist/:symbol", auth_1.authenticate, async (req, res) => {
    try {
        const userId = parseInt(req.params.userId);
...
app.post("/users/:userId/predictions", auth_1.authenticate, async (req, res) => {
    try {
        const userId = parseInt(req.params.userId);
